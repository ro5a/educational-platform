                ...
                @foreach ($users as $user)
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="dashboard?do=Edit&Id={{ $user->id }}" class="btn btn-icon btn-secondary"> <i
                                    class="bx bx-edit-alt me-1"></i></a>
                            <button class="btn btn-danger btn-buy-now" data-bs-toggle="modal"
                                data-bs-target="#deleteModal{{ $user->id }}"> <i class="bx bx-trash me-1"></i>
                            </button>

                            <x-drop-modal id='deleteModal{{ $user->id }}'>
                                <x-slot name='title'>حذف البيانات</x-slot>
                                <x-slot name='message'>هل انت متاكد انك تريد حذف بيانات البيانات</x-slot>
                                <x-slot name='link'>{{ route('dashboard.delete_user', $user->id) }}</x-slot>
                                <x-slot name='action'>
